...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!ship.success) {
      return NextResponse.json({ error: 'Invalid shipping address', details: ship.errors }, { status: 400 })
    }
    ...
    const saleorOrderId = ship.checkout.id // or ship.checkout.token if you need that

    // ✅ Return checkout info for Razorpay order creation
    return NextResponse.json({
      checkoutId,
      saleorOrderId,
      message: 'Checkout initialized. Proceed to payment.',
    })
  } catch (err) {
...
//below are the genuine payment proces where we can use payment through payment app above are just for demo use 
//in future after cloud upgradation we will use this
